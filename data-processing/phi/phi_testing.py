...
def perform_translations(code_df, start_lang, target_lang):
    ...
    for index, row in code_df.head(100).iterrows():
        ...
        while (retries < max_retries):
            ...
            if (response.status_code != 200):
                ...
                output_lines = [line if "//" not in line else line.split("//")[0] for line in output_lines]
                ...
                translation_info["source_code"] = str(start_lang_code)
                translation_info["reference_code"] = str(target_lang_code_truth)
                translation_info["translated_code"] = str(target_lang_code_pred)
    ...
    filename = f"./data-processing/phi/{lang_names[start_lang]}_to_{lang_names[target_lang]}.csv"
